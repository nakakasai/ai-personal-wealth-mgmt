...
async def categorize_transaction(desc: str, note: str, db: Session, user_id: int):
    ...
    for row in rows:
        try:
            ...
            txn = Transaction(
                ...
                amount=amount,
                category=category,
                source=SOURCE_SMBC_TRUST,
            )
            db.add(txn)

        except Exception as e:
            print("Skipping SMBC TRUST PDF row:", row, "Error:", e)
...
async def upload_file(
    ...
    try:
        if filename.endswith(".pdf") or statement_type in (
            "SMBC_TRUST",
            "BANK_PDF",
            "SMTRUST",
            "SMBC_TRUST_PDF",
            ...
            statement_type = SOURCE_SMBC_TRUST

        else:
            text = decode_csv_content(content)

            if statement_type == SOURCE_AMEX:
            ...
            elif statement_type == SOURCE_AEON:
            ...
            elif statement_type == SOURCE_ORICO:
            ...
            elif statement_type == SOURCE_OTHERS:
                stats = await parse_standard_csv(text, upload.id, user_id, db)

            else:
                raise HTTPException(
                    status_code=400,
                    detail="Unsupported statement type. Please select AEON, AMEX, ORICO, SMBC Trust, or Others.",
                )
